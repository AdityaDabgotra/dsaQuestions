...
// Optimal Approach
double merge3(vector<int>& arr1, vector<int>& arr2) {
   int s1 = arr1.size();
   int s2 = arr2.size();
   int s = s1+s2;

   if (s1 > s2) return merge3(arr2, arr1);
...
int main(){
   ...
   cout<<"The median of the two sorted array is:\t"<<merge3(array1,array2);
   return 0;
}